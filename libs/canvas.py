...
class Canvas(QWidget):
    ...
    def mouseMoveEvent(self, ev):
        ...
        high_vertex = False
        shapes = list(reversed([s for s in self.shapes if self.isVisible(s)]))
        if len(shapes)>0:
            for shape in shapes:
                ...
                if index is not None:
                    ...
                    high_vertex = True
                    break
            if high_vertex == False:
                shape = self.nearestShape(pos,shapes)
    ...
    def nearestShape(self,point,shapes=None):
        ...
        min_shape = None
        if shapes is None:
            shapes = reversed(self.shapes)
        for shape in shapes:
            if self.isVisible(shape) and shape.containsPoint(point):
                dis = self.shapeDistance(shape,point)
                if dis<min_dis:
                    min_dis = dis
                    min_shape = shape
        return min_shape

    def shapeDistance(self,shape,point):
